...
extension URLSessionTask {
    ...
    private var networkTrackerStateAccessor: AssociatedObjectAccessor<URLSessionTaskNetworkTrackerState> {
        .init(
            on: self,
            key: AssociatedKeys.networkTrackerState,
    ...
    private var networkTrackerState: URLSessionTaskNetworkTrackerState {
        if let state = existingNetworkTrackerState {
            return state
        }

        return AssociatedKeys.stateCreation.withLock { _ in
            if let state = existingNetworkTrackerState {
                return state
            }
    ...
    private var networkTrackerStateIfAvailable: URLSessionTaskNetworkTrackerState? {
        if let state = existingNetworkTrackerState {
            return state
        }

        return AssociatedKeys.stateCreation.withLockIfAvailable { _ in
            if let state = existingNetworkTrackerState {
                return state
            }
    ...
    func withNetworkTrackerState<Result>(
        ...
        try networkTrackerState.values.withLock(body)
    }

    func withNetworkTrackerStateIfAvailable<Result>(
        _ body: (inout URLSessionTaskNetworkTrackerState.Values) throws -> Result
    ) rethrows -> Result? {
        guard let state = networkTrackerStateIfAvailable else {
            return nil
        }
        return try state.values.withLockIfAvailable(body)
    }
